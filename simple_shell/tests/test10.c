...
{
	int num_commands;
	struct pipeline* my_pipeline = pipeline_build("ls|wc -l >counts.txt|ls|ls&\n", &num_commands);
    ...
    TEST_ASSERT(strcmp("ls", my_pipeline->commands->next->next->command_args[0]) == 0);
	TEST_ASSERT(my_pipeline->commands->next->next->command_args[1] == NULL);

    TEST_ASSERT(strcmp("ls", my_pipeline->commands->next->next->next->command_args[0]) == 0);
	TEST_ASSERT(my_pipeline->commands->next->next->next->command_args[1] == NULL);
    ...
    TEST_ASSERT(my_pipeline->commands->next->next->next->next == NULL);

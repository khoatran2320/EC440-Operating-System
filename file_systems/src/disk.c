...
static bool get_bit(unsigned short offset, uint8_t *bitmap)
{
...
static unsigned short find_free_bit(uint8_t *bitmap){
    for(unsigned short i = 0; i < sizeof(bitmap)*8; ++i){
        if(!get_bit(i, bitmap)){
            return i;
        }
    }
...
int fs_read(int fildes, void *buf, size_t nbyte)
{
    ...
    if(starting_block < NUM_DIRECT_BLOCKS){
        for(int i = starting_block; i < NUM_DIRECT_BLOCKS; ++i){
            if(inode_table[inode_idx].direct_block[i]){
    ...
    if(nbyte - bytes_read > 0 && inode_table[inode_idx].single_indirect_block){
...
{
    //write nbyte from buf to file referenced by fildes
    //check valid fildes
    //find inode
    //validate inode
    ...
    nbyte = (inode_table[inode_idx].file_size + nbyte) > (MAX_FILE_SIZE )? ((MAX_FILE_SIZE )- (inode_table[inode_idx].file_size)) : nbyte;
    int cursor = fd_table[fildes].offset;
    size_t bytes_written = 0;
    int current_ind_block = 0;

    /* write to direct blocks */
    while(bytes_written < nbyte && current_ind_block < NUM_DIRECT_BLOCKS){

        //skip the direct block if it's used to store current data offset by cursor
        if(inode_table[inode_idx].direct_block[current_ind_block]){
            ++current_ind_block;
            continue;
        }
        // find an available block in disk
    ...
    if(nbyte - bytes_written > 0){
        ...
        if(inode_table[inode_idx].single_indirect_block){
            ...
            for(ind_block_idx = 0; ind_block_idx < BLOCK_SIZE/2; ++ind_block_idx){
                if(ind_block[ind_block_idx] == 0){
                    break;
                }
            }
        }   
        ...
        for(;ind_block_idx < blocks_needed; ++ind_block_idx){
            ...
            //find free block
            unsigned short free_ind_block = find_free_bit(blocks_bitmap);
            if(!free_ind_block){
                inode_table[inode_idx].file_size += bytes_written;
                cursor += bytes_written;
                fd_table[fildes].offset = cursor;
        ...
        //find an available block to use as an indirect block
        // unsigned short free_block;
        // if(inode_table[inode_idx].single_indirect_block){
        //     free_block = inode_table[inode_idx].single_indirect_block;
        // }
        // else{
    ...
    if(bytes_written != nbyte){
        perror("fs_write: something weird happened");
        return -1;
    }
...
int fs_truncate(int fildes, off_t length)
{
    ...
    for(; starting_block < NUM_DIRECT_BLOCKS; ++starting_block){
        if(inode_table[inode_idx].direct_block[starting_block]){
            clear_bit(starting_block, blocks_bitmap);
        }
        else{
            break;
        }
